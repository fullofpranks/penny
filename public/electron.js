...
function createWindow() {
  ...
  // Spoof a modern Chrome user agent to avoid "app may not be secure" blocks
  try {
  ...
  // Enable content protection by default (hides window from screen capture)
  try { mainWindow.setContentProtection(true); } catch (e) {}
  ...
  // Open Google auth and other restricted domains in the system browser
  const openExternallyIfBlocked = (targetUrl) => {
    try {
      const u = new URL(targetUrl);
      const host = u.hostname;
      const isGoogleAuth = host.endsWith('accounts.google.com') || host.endsWith('oauth.googleusercontent.com');
      const isOAuthPath = /oauth|signin|login|consent/i.test(u.pathname);
      if (isGoogleAuth || isOAuthPath) {
  ...
  mainWindow.webContents.setWindowOpenHandler(({ url: targetUrl }) => {
    if (openExternallyIfBlocked(targetUrl)) {
      return { action: 'deny' };
    }
    return { action: 'allow' };
  });
...
app.on('ready', function () {
  createWindow();
  // Register global shortcuts (inspired by Ezzi)
  try {
    ...
    // Move window with arrow keys: Cmd/Ctrl + ↑ ↓ ← →
    const moveStep = 20;
    const moveBy = (dx, dy) => {
      if (!mainWindow) { return; }
    ...
    globalShortcut.register('CommandOrControl+Up', () => moveBy(0, -moveStep));
    globalShortcut.register('CommandOrControl+Down', () => moveBy(0, moveStep));
    globalShortcut.register('CommandOrControl+Left', () => moveBy(-moveStep, 0));
    globalShortcut.register('CommandOrControl+Right', () => moveBy(moveStep, 0));

    // Take screenshot: Cmd/Ctrl + H (save PNG to Pictures)
    globalShortcut.register('CommandOrControl+H', async () => {
      try {
        if (!mainWindow) { return; }
        const image = await mainWindow.capturePage();
        const picturesDir = app.getPath('pictures');
        const fileName = `pennywise_screenshot_${ Date.now() }.png`;
