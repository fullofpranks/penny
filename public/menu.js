...
function setMainMenu(mainWindow) {
  const appVersion = (app && typeof app.getVersion === 'function') ? app.getVersion() : 'dev';
  // Track content protection state locally (default enabled; see electron.js)
  let isContentProtected = true;
  const template = [
    {
      label: isWindows ? 'File' : (app && typeof app.getName === 'function' ? app.getName() : 'Pennywise'),
      submenu: [
        {
          label: 'Open',
    ...
    },
    {
      ...
      submenu: [
        ...getOpacityMenuItems(mainWindow),
        {
          ...
          click(menuItem) {
            isContentProtected = !!menuItem.checked;
            try { mainWindow.setContentProtection(isContentProtected); } catch (e) {}
          }
        },
        {
          label: 'Toggle Navbar',
    ...
    },
    {
      ...
      submenu: [
        ...
        },
        {
          label: 'Learn More',
          click() {
            shell.openExternal('https://github.com/kamranahmedse/kamranahmedse');
          }
        },
        {
          label: `About Version`,
